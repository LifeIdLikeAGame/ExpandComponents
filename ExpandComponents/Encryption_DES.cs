...
namespace ExpandComponents
{
    ...
    *********************************************************************************/
    ...
    /// 数据加密算法(DES)。此类无法被继承
    /// </summary>
    public sealed class Encryption_DES
    {
        ...
        public static string Encrypt(string value, string key, string iv = "")
        {
            ...
            if (key.Length < 8) throw new Exception("指定的密钥长度不能少于8位。");
            ...
            var _ivByte = string.IsNullOrEmpty(iv)==false ? Encoding.UTF8.GetBytes(iv.Substring(0, 8)) : _keyByte;
